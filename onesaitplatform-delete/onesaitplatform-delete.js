module.exports = function (RED) {
	...
	function OnesaitPlatformDelete(config) {
	  ...
	  function deleteQueryOntology(queryType, query, ontology, tries) {
			return new Promise((resolve, reject) => {
				function deleteQuery() {
					restClient.deleteByQuery(server.axiosAgent, {
						queryType,
						query,
						ontology
					}).then(response => {
		...
		node.on('input', function(msg) {
			if (server) {
				...
				if (protocol.toUpperCase() == 'REST'.toUpperCase()) {
					deleteQueryOntology(queryType, query, ontology, server.retries)
						.then(response => {
	...
	RED.nodes.registerType('onesaitplatform-delete', OnesaitPlatformDelete);
}